...
function buildBannerSection(movie) {
    ...
    div.innerHTML = `
            <h2 class="banner__title">${movie.title || movie.name || movie.original_name}</h2>
    ...
    return fetch(fetchUrl)
        ...
        .then(res => {
            ...
            if (Array.isArray(movies) && movies.length) {
                // Filter out movies that do not have a backdrop image
                const validMovies = movies.filter(m => m.backdrop_path !== null);
                buildMoviesSection(validMovies.slice(0, 6), categoryName);
            }
            return movies;
        })
        .catch(err => console.error(err))
...
function buildMoviesSection(list, categoryName) {
    ...
    const moviesListHTML = list.map(item => {
        return `
        <div class="movie-item" onmouseenter="searchMovieTrailer(${item.id}, 'yt${item.id}')" onmouseleave="clearMovieTrailer('yt${item.id}')">
...
function searchMovieTrailer(movieId, iframeId) {
    const elements = document.getElementById(iframeId);
    if (!elements || elements.innerHTML !== "") return;

    fetch(`${apiEndpoint}/movie/${movieId}/videos?api_key=${apikey}`)
        .then(res => res.json())
        .then(res => {
            ...
            if (trailer) {
                const div = document.createElement('div');
                div.innerHTML = `<iframe width="245px" height="150px" src="https://www.youtube.com/embed/${trailer.key}?autoplay=1&controls=0&mute=1" frameborder="0" allow="autoplay; encrypted-media" allowfullscreen></iframe>`;
                elements.append(div);
            }
        })
...
function clearMovieTrailer(iframeId) {
    const elements = document.getElementById(iframeId);
